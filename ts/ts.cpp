...
std::vector<int> global_client_sockets;
std::mutex mtx;
		...
		for (int i = 1; i < argc;)
		{
			...
			if (strcmp(argv[i], "-si") == 0)
			{
				...
				case 1:
					break;
				case 0:
					fprintf(stderr, "not a valid network address\n");
					return false;
				case -1:
					myerror("inet_pton");
					return false;
...
void addClient(int value)
{
	// 뮤텍스 잠그고
	mtx.lock();
	// 추가하고
	global_client_sockets.push_back(value);
	std::cout << "Client " << value << " is enter" << std::endl;
	// 잠금 해제
	mtx.unlock();
}

void removeClient(int value)
{
	// 뮤텍스 잠그고
	mtx.lock();

	// 제거하고
	auto it = std::remove(global_client_sockets.begin(), global_client_sockets.end(), value);
	global_client_sockets.erase(it, global_client_sockets.end());
	std::cout << "Client " << value << " is exit" << std::endl;
